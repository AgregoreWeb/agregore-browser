...
const {
	ExtensibleSession,
} = require("../../node_modules/electron-extensions/main");
...
class Extensions extends EventEmitter {
	...
	async listActions() {
		...
		return Object.keys(this.all)
			...
			.map(
				({ popupPage, backgroundPage, manifest, id, path: extensionPath }) => {
					...
					const iconRelative =
						typeof default_icon === "string"
							? default_icon
							: default_icon[19] || default_icon[38];
					const icon = new URL(
						path.join(extensionPath, iconRelative),
						"file:///"
					).href;
					return {
						id: name,
	...
	async registerAll(extensionsFolder = this.location) {
		...
		const stats = await Promise.all(
			rawNames.map((name) => fs.stat(path.join(extensionsFolder, name)))
		);

		const extensionFolders = rawNames.filter((name, index) =>
		...
		for (const folder of extensionFolders) {
			try {
				const extension = await this.loadExtension(
					path.join(extensionsFolder, folder)
				);
