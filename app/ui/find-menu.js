    ...
    `;
		...
		this.input.addEventListener("keydown", ({ keyCode, shiftKey }) => {
			if (keyCode === 13) {
				...
				this.dispatchEvent(
					new CustomEvent(direction, { detail: { value, findNext: true } })
				);
			}
		});
		...
		this.previousButton.addEventListener("click", () => {
			...
			this.dispatchEvent(
				new CustomEvent("previous", { detail: { value, findNext: false } })
			);
		});
		this.nextButton.addEventListener("click", () => {
			...
			this.dispatchEvent(
				new CustomEvent("next", { detail: { value, findNext: true } })
			);
		});
